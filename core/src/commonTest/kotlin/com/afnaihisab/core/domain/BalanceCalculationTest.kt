...
class BalanceCalculationTest {
    ...
    fun `AC-6 derives net balance from splits and settlements, owed minus owed minus settlements`() {
        ...
        val expense = testExpense(id = expenseId, ledgerId = ledgerId, payerMembershipId = m1.id, amount = 300L)
        val splits =
            listOf(
                testSplit(id = uuid(101), expenseId = expenseId, membershipId = m1.id, amount = 100L),
                testSplit(id = uuid(102), expenseId = expenseId, membershipId = m2.id, amount = 100L),
                testSplit(id = uuid(103), expenseId = expenseId, membershipId = m3.id, amount = 100L),
            )

        // Settlement: m2 pays m1 back 100 (fully settling m2's share).
        val settlements =
            listOf(
                testSettlement(id = uuid(200), ledgerId = ledgerId, fromMembershipId = m2.id, toMembershipId = m1.id, amount = 100L),
            )
        ...
        assertEquals(100L, byMembership.getValue(m1.id).netBalance, "m1 should still be owed 100 (from m3)")
        // m2: paid nothing, owes 100, paid a 100 settlement -> 0 - 100 + 100 = 0
        assertEquals(0L, byMembership.getValue(m2.id).netBalance, "m2 fully settled their 100 share")
        // m3: paid nothing, owes 100, no settlement -> 0 - 100 = -100
        assertEquals(-100L, byMembership.getValue(m3.id).netBalance, "m3 still owes 100")

        assertEquals(0L, balances.sumOf { it.netBalance }, "balances must always net to zero across all members")
    }
    ...
    fun `AC-7 a fully-settled ledger nets to exactly zero for every member, even with AC-2 rounding`() {
        ...
        // extra cent: 34/33/33.
        val expenseId = uuid(100)
        val expense = testExpense(id = expenseId, ledgerId = ledgerId, payerMembershipId = m1.id, amount = 100L)
        val splits =
            listOf(
                testSplit(id = uuid(101), expenseId = expenseId, membershipId = m1.id, amount = 34L),
                testSplit(id = uuid(102), expenseId = expenseId, membershipId = m2.id, amount = 33L),
                testSplit(id = uuid(103), expenseId = expenseId, membershipId = m3.id, amount = 33L),
            )
        assertEquals(100L, splits.sumOf { it.amount }, "fixture sanity check: splits must sum to the expense amount")

        // Every non-payer settles their exact share with the payer.
        val settlements =
            listOf(
                testSettlement(id = uuid(200), ledgerId = ledgerId, fromMembershipId = m2.id, toMembershipId = m1.id, amount = 33L),
                testSettlement(id = uuid(201), ledgerId = ledgerId, fromMembershipId = m3.id, toMembershipId = m1.id, amount = 33L),
            )

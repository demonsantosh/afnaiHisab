...
## System design (ADR-0022, ADR-0025 through ADR-0029)
...
- **Accepted risk**: provider outages (Koyeb, Neon, Vercel) are not mitigated by multi-region/multi-provider redundancy — this follows directly from ADR-0022's best-effort availability target, not an oversight. Revisit only if that NFR changes.
...
- **Operational limits** (ADR-0026): pagination capped (default 50, max 200), request body size limited, HTTP/DB/connection-acquisition timeouts set explicitly (not left unconfigured), graceful shutdown on `SIGTERM`.
- **Timezone handling** (ADR-0027): every stored/transmitted timestamp is UTC (`Instant`); localization is display-only, client-side, never server-side.
- **API compatibility** (ADR-0028): `/api/v1` is additive-only once mobile clients exist (Phase 3+) — a breaking change requires `/api/v2` with a deprecation window, decided concretely when it's actually needed, not speculated now.
- **Data-integrity reconciliation** (ADR-0029): a periodic check (manual or scheduled) that split-sums match expense amounts and every ledger's member balances net to zero — defense in depth independent of `core`'s write-path validation, catching a bypass (bad migration, manual DB fix, future admin tool), not a bug in `core` itself.

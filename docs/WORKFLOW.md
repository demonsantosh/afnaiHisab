...
| A whole feature/milestone spanning multiple steps and/or both `core`/`server` and `web` | `afnaihisab-orchestrator` (`.claude/agents/`) | Plans the feature into tracked steps (TodoWrite), delegates each step to `afnaihisab-backend`/`afnaihisab-web`, does the cross-cutting work (specs, reconciliation, `STATUS.md`) itself, and drives the whole thing to a verified finish rather than stopping after scoping it |
